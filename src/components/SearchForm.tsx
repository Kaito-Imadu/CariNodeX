...
interface SearchFormProps {
  ...
  initialSizes?: CompanySize[];
}

export function SearchForm({ onSearch, isLoading, initialKeywords = "", initialSizes = [] }: SearchFormProps) {
  const [keywords, setKeywords] = useState(initialKeywords);
  const [selectedSizes, setSelectedSizes] = useState<CompanySize[]>(initialSizes);
                  ...
                  <div className="px-3 pb-2.5 flex flex-wrap gap-1.5 animate-fade-in">
                    {cat.keywords.map((kw) => (
                      <Tag
                        key={kw}
                        label={kw}
                        selected={keywords.split("、").map((s) => s.trim()).includes(kw)}
